...
- implementation_status: bot_update_runtime_deployed_account_management_code_complete_e3_passed
...
| Account management Bot flow | `src/tg_v_chat/bot/account_management.py` | dev | Renders Account Management home/list/detail/status/help, tracks bind wizard steps, and routes phone/code/2FA inputs |
...
| Telegram adapters | `src/tg_v_chat/telegram/telethon_clients.py` | dev | Telethon bot process, inline callback handling, real user-session authenticator, gateway/sender ports; disconnected adapters raise explicit errors |
...
| Bot account flow | `src/tg_v_chat/bot/account_management.py` | dev | Renderer and stateful handler for home, empty state, account list, account detail, status, help, binding prompts, cancel, and disable confirmation |
...
| Telegram authenticator | `src/tg_v_chat/telegram/telethon_clients.py` | dev | Real user-session auth adapter for send-code, complete-code, complete-password, and session export |
