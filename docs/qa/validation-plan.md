...
| native-forward-bridge-isolation | req-native-forward-relay-v2 | none | E3/E4 | 同一 telegram_user_id 的 marker、NewMessage、Album 在同一 sender lock 内串行；有效 marker 后每个 item 按 Bot 私聊 message id 顺序写入下一个未 bridge item，且 bridge id 仅在 sender/private-chat scope 唯一；重复 id 幂等，错误 marker、orphan 和超过批次条数的 forwarded item quarantine，绝不进入普通 router | E3 延迟 marker commit、并发 item/album、双账号相同 Bot message id、重复/超过条数 quarantine 与 router-spy 测试；E4 多账号并发真实观察 | unproven |
| native-forward-first-hop-liveness | req-native-forward-relay-v2 | none | E3/E4 | 第一跳仅返回 `forwarded_count`，必须等于 expected_count；发送侧 marker/item id 不持久化，Bot 私聊 marker/item id 是唯一关联键；bridge_sending/awaiting_bot 到 `bridge_deadline_at` 进入 bridge_timeout/uncertain 并通知，不自动重放 | E3 adapter count、Bot marker 先到回归、时钟/reconciliation、重启和通知测试；E4 临时断开/受控延迟验证 | unproven |
