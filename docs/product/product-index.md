...
| BoundTgAccount | binding, active, degraded, reauth_required, disabled | 归属 SystemUser | TG Bot binding flow | auth flow, session listener | primary usable 为 active；primary 不可用但 standby 可用为 degraded；全部已授权槽不可用为 reauth_required；未授权 standby 不触发降级。 |
