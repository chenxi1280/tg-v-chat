...
class AuthService:
    ...
    def start_binding(self, telegram_user_id: int, phone_number: str, slot: DeveloperSlot) -> AuthChallenge:
        ...
        challenge = self._uow.auth_challenges.create(account.id, phone_number, slot, external.phone_code_hash)
        self._uow.commit()
    ...
    def _create_auth_slot(self, account_id: int, active_slot: DeveloperSlot, encrypted: str, slot: DeveloperSlot) -> None:
...
def _challenge_from_model(model) -> AuthChallenge:
